...
export const Home = () => {
  ...
  return locationAllow.allow ? (
    <Container>
      <CustomTextField
        label="Search"
        placeholder="Search by city"
        onChange={(e: React.ChangeEvent<HTMLInputElement>) => getSearchText(e.target.value)}
        value={inputValue}
        fullWidth
      />
      ...
      {data && cardBGColor ? (
        <>
          {currentLocation && (
            <Center>
              <CustomTypography variant="h5" label={`Current Location Data`} />
          ...
          <CardElement variant="elevation" cardBGColorProp={cardBGColor}>
            <CardContent>
              <Center>
                <div>
                  <StyledBoldFont>{`Weather Condition - ${data.weather[0].main}`}</StyledBoldFont>
                </div>
                <img
                ...
                <CustomButton
                  size="medium"
                  label="View Historical Weather Report"
                  variant="contained"
                  color="info"
                  onClick={onClickViewHistorical}
                />
              </Center>
            </CardContent>
          </CardElement>
